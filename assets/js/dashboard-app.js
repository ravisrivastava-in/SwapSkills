/**
 * ============================================================
 * SwapSkills — dashboard-app.js
 * All dashboard UI logic (tabs: Dashboard, Skills, Matches, Chat)
 * Requires window.SS from firebase-app.js
 * ============================================================
 */
...
export async function init() {
    ...
    listenToMatches(UID, async matches => {
      ...
      initChatNotifications(UID, acc, (total, perMatch) => {
        ...
        document.querySelectorAll(".ch-c").forEach(el => {
          ...
          if (b) {
            if (cnt > 0) { b.textContent = cnt; b.classList.remove("d-none"); }
    ...
    el.innerHTML = acc.map(m => {
      ...
      return `<div class="chat-contact ch-c ${aMid === m.matchId ? 'active' : ''}" data-m="${m.matchId}">
        <div class="d-flex align-items-center gap-2 position-relative">
          ...
          <span class="contact-badge notif-badge d-none position-absolute" style="top:-2px;right:4px">0</span>
        </div></div>`;
    }).join("");
  ...
  // Logout
  $("logout-btn").onclick = async () => { await logoutUser(); location.href = "index.html"; };
